...
const (
	defaultServiceNode   = "default"
	defaultServiceRegion = "default"
...
func (s *StateStorage) serviceMatches(
	...
	for key, values := range selector {
		...
		case "Metadata.Node":
			...
			if actual == "" {
				actual = defaultServiceNode
			}
		case "Metadata.Region":
			actual = srv.Metadata["Region"]
			if actual == "" {
				actual = defaultServiceRegion
			}
		case "Metadata.Zone":
			actual = srv.Metadata["Zone"]
			if actual == "" {
				actual = defaultServiceZone
			}
		default:
			continue
		}
