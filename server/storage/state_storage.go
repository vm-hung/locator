...
func (s *StateStorage) Retrieve(selector map[string][]any) ([]*types.Service, error) {
	components := []string{
		"Metadata.Namespace", "Metadata.Region", "Metadata.Zone",
		"Metadata.SubZone", "Address.Host", "Address.Port",
	}
...
func (s *StateStorage) buildServiceKey(srv *types.Service) []byte {
	node := srv.Metadata.GetNamespace()
	region := srv.Metadata.GetLocality().Region
	zone := srv.Metadata.GetLocality().Zone
	subZone := srv.Metadata.GetLocality().SubZone

	return fmt.Appendf(
		nil, serviceRawPrefix+":%s:%s:%s:%s:%s:%d", node, region,
		zone, subZone, srv.Address.Host, srv.Address.Port,
	)
}
...
func (s *StateStorage) serviceMatches(
	...
	for key, values := range selector {
		...
		case "Metadata.Namespace":
			actual = srv.Metadata.GetNamespace()
		case "Metadata.Region":
			actual = srv.Metadata.GetLocality().Region
		case "Metadata.Zone":
			actual = srv.Metadata.GetLocality().Zone
		case "Metadata.SubZone":
			actual = srv.Metadata.GetLocality().SubZone
		default:
			continue
		}
