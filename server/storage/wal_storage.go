...
func (s *WalStorage) NewMemoryStorage() (*raft.MemoryStorage, error) {
	...
	if val, closer, err := s.db.Get([]byte(keyHardState)); err == nil {
	...
	if val, closer, err := s.db.Get([]byte(keySnapshot)); err == nil {
	...
	iter, _ := s.db.NewIter(&pebble.IterOptions{
		...
		UpperBound: upperbound([]byte(prefixEntry + ":")),
	})
...
func (s *WalStorage) Save(hs raftpb.HardState, entries []raftpb.Entry) error {
	...
	if !raft.IsEmptyHardState(hs) {
		if data, err := hs.Marshal(); err != nil {
			...
			if err := batch.Set([]byte(keyHardState), data, pebble.NoSync); err != nil {
				return err
			}
		}
	}
...
func (s *WalStorage) SaveSnapshot(snap raftpb.Snapshot) error {
	if data, err := snap.Marshal(); err == nil {
		return s.db.Set([]byte(keySnapshot), data, pebble.Sync)
	} else {
		panic("marshal snapshot must not fail")
	}
}

func (s *WalStorage) TruncateEntries(snap raftpb.Snapshot) error {
	startKey, endKey := s.encodeIndex(0), s.encodeIndex(snap.Metadata.Index)
	if err := s.db.DeleteRange(startKey, endKey, pebble.Sync); err != nil {
		log.Printf("failed to delete range %s - %s: %v", startKey, endKey, err)
		return err
	}
	return nil
}
