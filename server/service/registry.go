...
func (s *RegistryServer) Retrieve(
	...
	if node, ok := selector["Metadata.Node"]; !ok || len(node) == 0 {
		selector["Metadata.Node"] = []any{types.DefaultNamespace}
	}
...
func (s *RegistryServer) Register(
	...
	if node := service.Metadata["Namespace"]; node == "" {
		service.Metadata["Namespace"] = types.DefaultNamespace
	}
	if region := service.Metadata["Region"]; region == "" {
		service.Metadata["Region"] = types.DefaultRegion
	}
	if zone := service.Metadata["Zone"]; zone == "" {
		service.Metadata["Zone"] = types.DefaultZone
	}
	if subZone := service.Metadata["SubZone"]; subZone == "" {
		service.Metadata["SubZone"] = types.DefaultSubZone
	}
	...
	existing, err := s.locatorServer.Retrieve(
		ctx, map[string][]any{
			"Address.Host":       {service.Address.Host},
			"Address.Port":       {service.Address.Port},
			"Metadata.Namespace": {service.Metadata["Namespace"]},
			"Metadata.Region":    {service.Metadata["Region"]},
			"Metadata.Zone":      {service.Metadata["Zone"]},
			"Metadata.SubZone":   {service.Metadata["SubZone"]},
		},
	)
