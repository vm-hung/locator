...
import (
	...
	transportpb "github.com/hunkvm/locator/pkg/proto/transport"
...
type MessageProcessor interface {
	...
	Recover(context.Context, *transport.SnapshotMessage) error
}

type TransportServer struct {
	Processor MessageProcessor
	transportpb.UnimplementedRaftTransportServiceServer
}
...
func (s *TransportServer) Send(
	ctx context.Context, req *transportpb.SendMessageRequest,
) (*transportpb.SendMessageResponse, error) {
	var message raftpb.Message
	if err := message.Unmarshal(req.Data); err != nil {
		return &transportpb.SendMessageResponse{Success: false}, err
	}
	if err := s.Processor.Process(ctx, message); err != nil {
		return &transportpb.SendMessageResponse{Success: false}, err
	}
	return &transportpb.SendMessageResponse{Success: true}, nil
}

func (s *TransportServer) Stream(
	stream transportpb.RaftTransportService_StreamServer,
) error {
	...
	msg := transport.NewSnapshotMessage(message, pipeReader)
	...
	case ok := <-msg.CloseNotify():
		res := &transportpb.StreamDataResponse{Success: ok}
